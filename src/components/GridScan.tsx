...
function smoothDampVec2(current: any, target: any, currentVelocity: any, smoothTime: any, maxSpeed: any, deltaTime: any) {
...
function smoothDampFloat(current: any, target: any, velRef: any, smoothTime: any, maxSpeed: any, deltaTime: any) {
  smoothTime = Math.max(0.0001, smoothTime);
  const omega = 2 / smoothTime;
  const x = omega * deltaTime;
  const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x);
...
function medianPush(buf: any[], v: any, maxLen: number) {
  buf.push(v);
  if (buf.length > maxLen) buf.shift();
}

function median(buf: any[]) {
